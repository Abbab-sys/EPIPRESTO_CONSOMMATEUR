...
import {
  IS_CLIENT_EMAIL_USED,
  IsClientEmailUsedData,
} from '../../graphql/queries/IsClientEmailUsed';
import {MANDATORY_FIELD_EMPTY} from '../../types/errors/MandatoryFieldsErrors';

/*
 * Name: Use Email Validator
 * Description: This is a custom hook that validates the email.
...
export const useEmailValidator = (email: string) => {
  ...
  const isClientEmailUsedData: IsClientEmailUsedData | undefined =
    data as IsClientEmailUsedData;

  const validateEmail = (email: string): boolean => {
    return /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(email);
  };

  //Use effect to validate the email
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (
      isClientEmailUsedData?.isClientEmailUsed &&
      errors.indexOf(EMAIL_USED) === -1
    ) {
      setErrors(prevErrors => {
        return [...prevErrors, EMAIL_USED];
      });
