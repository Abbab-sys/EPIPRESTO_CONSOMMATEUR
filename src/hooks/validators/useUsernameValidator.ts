...
/*
 * Name: Use Username Validator
 * Description: This is a custom hook that validates the username.
...
export const useUsernameValidator = (username: string) => {
  ...
  //Use effect to validate the username
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (
      isClientUsernameUsedData?.isClientUsernameUsed &&
      errors.indexOf(USERNAME_USED) === -1
    ) {
      setErrors(prevErrors => {
        return [...prevErrors, USERNAME_USED];
      });
