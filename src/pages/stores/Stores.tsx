...
/*
 * Name: Stores
 * Description: This file is used to display the list of stores page.
 * Author: Ryma Messedaa, Alessandro van Reusel, Zouhair Derouich, Adam Naoui-Busson
 */
...
const Stores = ({route, goBack, navigation}: any) => {
  ...
  // Return the category of the store from an index
  ...
  const {loading, error, refetch} = useQuery(GET_CLIENT_ACCOUNT_BY_ID, {
    ...
    onCompleted(data) {
      ...
      const formattedStores = stores.map((store: any) => {
        const disponibilities = store.disponibilities.map(
          (disponibility: any) => {
            if (disponibility.activesHours.length < 1) return null;
            // consider multiple actives hours
            const activesHours = disponibility.activesHours.map(
              (activeHour: any) => {
          ...
          <FlatList
            ...
            renderItem={({item}) => (
              <View style={productStyles.root}>
                <Card
                  style={[
                    storeStyles.cardStyle,
                    item.isPaused ? {opacity: 0.6} : {opacity: 1},
                  ]}>
                  <View
                    style={{
                      flexDirection: 'row',
                  ...
                  <Text variant="labelMedium" style={{textAlign: 'center'}}>
                    {t('shopCategories.' + item.shopCategory)}
                  </Text>
                  <View
                    style={{
                      flexDirection: 'row',
